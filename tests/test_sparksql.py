...
def df_ts():
    return mock.MagicMock(
        columns=["col1"],
        take=lambda n: [
            Row(col1=datetime.datetime(1, 1, i, 0, 0, 0)) for i in range(1, n)
        ],
    )
...
def test_bind_variables(user_ns):
    ...
    assert (
        bind_variables(
            "SELECT * FROM {{schema}}.table WHERE lsh_dtm between {{start_var}} and {{end_var}}",
            user_ns,
        )
        == "SELECT * FROM dl.table WHERE lsh_dtm between '2019-05-01' and '2019-05-31'"
    )
...
def test_make_tag():
    ...
    assert (
        make_tag("td", "body", style="font-weight: bold")
...
def test_timestamp_with_timezone(df_ts):
    ...
    assert get_results(df_ts, 2) == (
        ["col1"],
        [["0001-01-01 00:00:00"], ["0001-01-02 00:00:00"]],
    )
